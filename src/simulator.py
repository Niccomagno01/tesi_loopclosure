...
class Simulator:
    def __init__(self, robot=None):
        ...
        self.commands = None  # Conterrà la sequenza di comandi (v, omega) puliti
        self.commands_applied = None  # comandi realmente applicati al robot (disturbati)

    def run_from_sequence(self, vs, omegas, dt, vs_applied=None, omegas_applied=None):
        ...
        self.commands = np.zeros((n, 2))  # Array per salvare i comandi (v, omega) puliti
        self.commands_applied = np.zeros((n, 2)) # Array per salvare i comandi realmente applicati (disturbati)
        self.history[0] = self.robot.state()  # Salva lo stato iniziale del robot come prima riga della history

        if vs_applied is None:
            vs_applied = vs
        if omegas_applied is None:
            omegas_applied = omegas

        for k in range(n):  # Itera su ogni passo temporale
            self.robot.set_command(vs_applied[k], omegas_applied[k])  # Imposta i comandi di velocità lineare e angolare per questo step
            ...
            self.commands[k] = [vs[k], omegas[k]]  # Memorizza i comandi nominali (puliti) per l'odometria
            self.commands_applied[k] = [vs_applied[k], omegas_applied[k]] # Memorizza i comandi realmente applicati (disturbati)
    ...
    def reset_robot(self, x=0.0, y=0.0, theta=0.0):
        ...
        self.commands_applied = None  # Azzera i comandi applicati
